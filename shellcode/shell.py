...
if (N_part % 2 == 0):
 ...
 for i in SD_M_fix: 
  for j in range(N_part/2):
    if (n_inp[Index_inp.index(np.split(np.asarray(i),N_part/2)[j][0])] == n_inp[Index_inp.index(np.split(np.asarray(i),N_part/2)[j][1])] and 
        l_inp[Index_inp.index(np.split(np.asarray(i),N_part/2)[j][0])] == l_inp[Index_inp.index(np.split(np.asarray(i),N_part/2)[j][1])] and
        j_inp[Index_inp.index(np.split(np.asarray(i),N_part/2)[j][0])] == j_inp[Index_inp.index(np.split(np.asarray(i),N_part/2)[j][1])]):
...
np.savetxt("Hmatrix.txt", Hmat, fmt="%.5f")
...
#print Update_SD1(SD_M_fix[0],1,4,1,4)

#print perm_sorted(Update_SD1(SD_M_fix[0],1,4,1,4))

#print Update_SD1(SD_M_fix[0],1,4,1,4).sort()
